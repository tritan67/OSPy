...
from ospy.urls import urls # Gain access to ospy's URL list
from ospy.webpages import ProtectedPage, WebPage
...
class station_info(ProtectedPage):  # /jn
    ...
    #           disable.append(~byte&255)
        ...
        jpinfo = {
            ...
            "stn_dis": [0],
            "maxlen": 32
            # not used in refactor? gv.sd['snlen'] max size of station names 32
        }
    ...
    def GET(self):
        ...
        for interval in events:
            ...
            timestamp = int(time.mktime(datetime.datetime.strptime(
                interval['start'].strftime("%Y-%m-%d") + " " + interval['start'].strftime("%H:%M:%S"),
